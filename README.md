...
- 🌒 **Modern dark UI** — PySide6 + QDarkStyle, with a *Performance* readout of three live sections (pipeline FPS, camera FPS and inference latency) plus handedness and confidence indicators. Every bar is shaded from green to red as its value gets worse; the two confidence bars start their scale at the thresholds the recognizer is running with.
...
4. **Output** — the annotated frame, recognized letter, confidence and the three *Performance* sections (pipeline FPS, camera FPS and inference latency) are rendered in the GUI; the letter is optionally synthesized to speech.
...
4. **Smoothing** — press the *Smoothing* button to average the last *N* results (window size set with the *Window* slider) for a more stable output.
...
In low light a webcam with auto-exposure lengthens its exposure time and silently divides its frame rate — the device still advertises 30 FPS while it delivers only about 8–10 unique frames per second, and the recognition rate follows. The *Performance* group separates the two possible causes into its own sections: **Camera** (how fast frames arrive) and **Inference** (how long one recognition takes). Each section's bar turns from green through amber to red as its value gets worse.
