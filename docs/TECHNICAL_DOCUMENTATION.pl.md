...
- **Odzyskiwanie i backpressure** — w danej chwili może oczekiwać tylko jedno `recognize_async()`; klatki przechwycone w trakcie trwającej inferencji są porzucane, a nie kolejkowane. Callback kolejkuje kolejne wysłanie w wątku Qt, nieudane wysłanie jest ponawiane po 50 ms, a nieudany odczyt jest ponawiany przez wątek przechwytywania po 50 ms — w żadnym z tych przypadków GUI nie jest blokowane. Wysłany pakiet, na który nigdy nie przyjdzie odpowiedź — MediaPipe może go odrzucić w swoim ograniczniku przepływu — pozostawiłby flagę trwającej inferencji ustawioną na zawsze, dlatego jest porzucany po czasie `max(1 s, 10 × średnie opóźnienie)`, a jego miejsce zajmuje kolejna klatka. Wysłania są kluczowane znacznikiem czasu w milisekundach, więc wynik, który mimo wszystko dotrze dla porzuconego pakietu, jest rozpoznawany jako spóźniony i odrzucany: jego księgowanie należy już do wysłania, które go zastąpiło, a jego klatka jest starsza od aktualnie rozpoznawanej. Odrzucana jest wyłącznie klatka: pakiet nadal raportuje swój czas wykonania, ponieważ recognizer przechowuje czasy startu kilku ostatnich wysłań, a zasilana nimi średnia krocząca podnosi limit czasu. Mierzenie samych przyjętych pakietów utrzymywałoby limit na wartości minimalnej, więc model wolniejszy niż jedna sekunda miałby porzucane wszystkie wysłania i nigdy nie zwróciłby wyniku; dzięki temu pierwsza spóźniona odpowiedź rozszerza limit i potok się stabilizuje. Pamiętanych jest tylko kilka ostatnich wysłań, więc pakiet, który odpowie jeszcze później, mierzony jest względem najstarszego pamiętanego wysłania — jest to rzeczywiste ograniczenie dolne wzięte z zegara, nigdy z samego limitu, który wyliczany jest z tych właśnie pomiarów i w przeciwnym razie rósłby bez ograniczeń. Dodatkowo limit jest ograniczony do 60 s, ponieważ czekanie minutami na jeden callback nie ma sensu: model wolniejszy niż ten pułap jest po prostu wysyłany ponownie raz na pułap, zamiast zamrażać potok do końca sesji.
- **Pomiar wydajności** — trzy odrębne liczby, wszystkie z zegarów monotonicznych i wszystkie raportowane z dokładnością do dziesiętnej: **FPS potoku** (wyniki rozpoznawania na sekundę, `calculate_fps`, średnia krocząca z odstępów między ostatnimi 30 wynikami), **FPS kamery** (ta sama średnia krocząca z odstępów między klatkami dostarczanymi przez wątek przechwytywania) oraz **opóźnienie inferencji** w milisekundach (od `recognize_async()` do wejścia w callback, średnia krocząca). Pojedyncza liczba nie pozwalała odróżnić wolnej kamery od wolnego modelu. Okno potoku jest czyszczone przy każdym starcie przechwytywania, więc przerwa nigdy nie wchodzi do średniej.
- **Zatrzymywanie przechwytywania** — `stop_capture()` prosi wątek o zakończenie i czeka na niego (limit 2 s). Każda ścieżka, która ponownie otwiera, konfiguruje, otwiera natywne ustawienia lub zwalnia kamerę, przechodzi najpierw przez nią — reset kamery, zmiana rozdzielczości, reset rozpoznawania, zmiana modelu, okno ustawień i zamykanie aplikacji — i żadna z nich nie dotyka urządzenia, gdy oczekiwanie się nie powiodło; dzięki temu kamera nigdy nie jest jednocześnie czytana i otwierana, a żaden callback nie trafia do zwolnionego obiektu.
- **Sterownik, który nie wraca** — jeśli `read()` nie wróci w zadanym czasie, wątek jest celowo pozostawiany bez zwolnienia: błąd jest logowany, odczyt pokazuje *camera busy, retry* — komunikat używany na każdej ścieżce, na której wątek wciąż trzyma urządzenie, w odróżnieniu od *camera not running*, gdy przechwytywania w ogóle nie udało się uruchomić — a `CameraApp` odmawia ponownego otwarcia, konfiguracji i zwolnienia tego uchwytu tak długo, jak długo porzucony wątek działa (`camera_is_stranded`) — wyrwanie mu przechwytywania przerwałoby proces. Ponowne uruchomienie jest odrzucane natychmiast, a nie po kolejnym oczekiwaniu, więc GUI zatrzymuje się najwyżej na jeden limit 2 s, a następny *Reset camera* powiedzie się, gdy sterownik zwolni wątek. Przy wyjściu `main.shutdown()` celowo zostawia uchwyt kamery i kończy proces bez uruchamiania finalizatorów interpretera, ponieważ niszczenie działającego `QThread` w trakcie finalizacji przerywa program kodem `0xC0000409`.
...
Konfiguruje `logging` (poziom INFO, UTF-8), a następnie mierzy ekran, zanim powstanie cokolwiek z Qt: `work_area_metrics()` odczytuje obszar roboczy i systemowe DPI przez `user32`, a `configure_scaling()` ustawia `QT_SCALE_FACTOR` na podstawie `scale_factor_for()`, gdy okno o stałym rozmiarze by się nie zmieściło — tę zmienną Qt czyta tylko w chwili tworzenia obiektu aplikacji, a wartość ustawioną przez użytkownika pozostawia bez zmian. Dalej tworzy `QApplication`, nakłada arkusz stylów QDarkStyle (`qt_api='pyside6'`, `DarkPalette`), tworzy instancję `MainApp`, wywołuje `start()` i pokazuje okno.

Po pokazaniu okna `fit_to_screen()` przesuwa jego ramę w obszar roboczy ekranu (do lewego górnego rogu, jeśli nadal się nie mieści), a `clear_initial_focus()` zabiera klawiaturę polom ustawień, dzięki czemu aplikacja startuje bez podświetlonego pola. `shutdown()` kończy proces po powrocie z pętli zdarzeń Qt.
...
| `update_recognition_rate(metrics)` | Aktualizuje trzy sekcje *Performance*: FPS potoku, FPS kamery i opóźnienie inferencji, każda z własnym paskiem |
| `update_rate_bar(bar, value, badness)` | Ogranicza pomiar do zakresu paska i koloruje pasek od zielonego do czerwonego, nakładając arkusz stylów tylko przy zmianie koloru |
...
- `start()` / `stop(timeout=2.0)` — uruchamia pętlę przechwytywania albo prosi ją o zakończenie i czeka na wątek; obie metody zwracają `False`, jeśli wątek nie zakończy się w zadanym czasie. Zatrzymany wątek można uruchomić ponownie, również wtedy, gdy poprzednie `stop()` przekroczyło limit czasu, a jego pętla dopiero się kończy.
- `capture_once()` — odczytuje jedną klatkę, aktualizuje kroczące `camera_fps` i publikuje klatkę. Zwraca `False`, gdy kamera jest zamknięta lub odczyt się nie powiódł — pętla robi wtedy 50 ms przerwy zamiast kręcić się w kółko; wyjątek ze sterownika jest logowany, a pętla działa dalej, bo wyjątek opuszczający `run()` po cichu zabiłby wątek przechwytywania.
...
- `LatestFrameBuffer` — jednoelementowy, chroniony blokadą bufor wymiany między wątkami. `put()` nadpisuje niepobraną klatkę i zlicza ją w `dropped_frames` (licznik zbiorczy od startu), `take()` zwraca najnowszą klatkę albo `None`, a `clear()` porzuca ją bez zliczania.
...
- `start_capture()` / `stop_capture()` tworzą, uruchamiają i zatrzymują `CameraWorker`; `stop_capture()` czyści też bufor klatek, więc wznowiony potok nigdy nie startuje od nieaktualnej klatki. Obie zwracają `False`, gdy wątku przechwytywania nie udało się uruchomić lub zatrzymać — podobnie jak `close()`, na podstawie którego okno decyduje, czy zwolnienie kamery jest bezpieczne.
...
- `metrics()` zwraca `PipelineMetrics(pipeline_fps, camera_fps, inference_ms, dropped_frames)` — liczby prezentowane w trzech sekcjach *Performance*.
...
Okno zawiera podgląd wideo (`label_displayFrame`, 640×480), wiersz ustawień (kamera, rozpoznawanie, TTS, wygładzanie) oraz cztery grupy najwyższego poziomu po prawej stronie: *Results* `groupBox_5` z rozpoznanym znakiem i paskiem pewności; *Hand* `groupBox_8` z ręcznością (`label_recognitionInfo`, *Left* / *Right* / *None*) i jej paskiem pewności; *Performance* `groupBox_performance` z trzema zagnieżdżonymi ramkami grup, których tytuł niesie nazwę metryki, każda z etykietą wartości i paskiem — *Recognition rate* `groupBox_pipelineRate` (`label_displayFPS` / `progressBar_fps`), *Camera* `groupBox_cameraRate` (`label_displayCameraFps` / `progressBar_camera_fps`) i *Inference* `groupBox_inference` (`label_displayInference` / `progressBar_inference`); oraz *Author*. Każdy pasek w oknie jest kolorowany od zielonego przez bursztynowy do czerwonego zależnie od tego, jak zła jest jego wartość (`rate_colour` w `src/main_app.py`): ramki wydajności względem stałych skal (`fps_badness`, `inference_badness`), a oba paski pewności względem progów ustawionych w *Settings* (`confidence_badness`) — skala paska znaku zaczyna się od progu *Threshold*, a paska dłoni od *Presence*, obie brane z działającego rozpoznawania, więc nowy próg zmienia kolory dopiero po zatwierdzeniu przyciskiem **Reset**. Kolor tekstu w pasku jest przełączany między niemal czarnym a niemal białym zależnie od jasności tła, na którym wypada (`text_colour_for`): wypełnienia przy odczycie powyżej połowy i ciemnego rowka poniżej. Liczba porzuconych klatek jest podpowiedzią ramki *Camera* (`groupBox_cameraRate`).

## 5. Potok treningu modelu

Własny model trenowany jest w Google Colab przy użyciu **MediaPipe Model Maker** (notatnik: [`notebooks/Custom_gesture_recognizer.ipynb`](../notebooks/Custom_gesture_recognizer.ipynb)).
...
- Trzy sekcje *Performance* pozwalają odróżnić obie przyczyny: niski odczyt **Camera** przy niskim czasie **Inference** wskazuje na ekspozycję i oświetlenie, a nie na model czy procesor.
